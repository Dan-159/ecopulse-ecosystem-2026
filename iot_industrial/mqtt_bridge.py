...
def on_message(client, userdata, msg):
    try:
        ...
        id_zona = msg.topic.split('/')[-1]

        # Actualizar última vez vista
        last_seen[id_zona] = time.time()

        data_to_send = {
            "id_zona": int(id_zona),
            "lectura_de_co2": payload["lectura_de_co2"],
            "lectura_de_nox": payload["lectura_de_nox"],
            "lectura_de_pm25": payload["lectura_de_pm25"]
        }
        ...
        try:
            ...
            if response.status_code == 201:
                print(f"✅ Dato persistido en DB para la zona {id_zona}")
            else:
                print(
...
def check_deadlines():
    while True:
        ...
        for id_zona, last_time in list(last_seen.items()):
            if current_time - last_time > 30:
                print(f"🚨 ALERTA: Zona {id_zona} está OFFLINE")
